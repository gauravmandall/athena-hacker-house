...
class SuperAggressiveDrivingPolicy extends BaseDrivingPolicy {
  ...
  private getTarget(current_position: Vec2D): {
    ...
    if (
      ...
      const nextPlayerStep = Vector.generateVectorFromAngle(
        this.distanceBeforePlayer,
        playerDirection
      );
      ...
      const isDegreeGood =
        Vector.degreeBetweenVectors(
          Vector.subtract(positionBeforePlayer, current_position),
          nextPlayerStep
      ...
      if (isCloseEnough && isDegreeGood && isntTooClose) {
        ...
        return {
          ...
          target: {
            point: positionBeforePlayer,
            curvature: 0,
            tangent: positionBeforePlayer,
          },
        };
      }
    }
